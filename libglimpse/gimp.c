           ...
           gchar                *argv[])
{
  ...
  if (env_string)
    {
      ...
      if (debug_messages)
        {
          gchar *tmp = g_strconcat (debug_messages, ",libglimpse", NULL);
          g_setenv ("G_MESSAGES_DEBUG", tmp, TRUE);
          g_free (tmp);
        }
      else
        {
          g_setenv ("G_MESSAGES_DEBUG", "libglimpse", TRUE);
        }
    }
  ...
  /* set handler both for the "libglimpse" and "" domains */
  {
 ...
 * gimp_quit:
 *
 * Forcefully causes The Glimpse Library to exit and close down its
 ...
 * the return values. Plug-ins that are using the libglimpse C wrappers
 * don't access the procedure return values directly. Thus libglimpse
 * stores the error message and makes it available with this
 * function. The next procedure call unsets the error message again.
 *
 * The returned string is owned by libglimpse and must not be freed or
 * modified.
 *
